...
\textbf{One-vs-One Classifier:} Este enfoque consiste en entrenar un modelo de clasificación binaria para cada par de clases en el dataset. En este caso, dado que tenemos 3 clases (Premium Lager, IPA, Light Lager), este enfoque entrenaría 3 modelos de clasificación binaria:
\begin{itemize}
    \item Premium Lager vs IPA
    \item Premium Lager vs Light Lager
    \item IPA vs Light Lager
\end{itemize}
...
\textbf{One-vs-Rest Classifier:} Este enfoque consiste en entrenar un modelo de clasificación binaria para cada clase en el dataset, donde cada modelo se entrena para distinguir una clase específica de todas las demás clases combinadas. En este caso, dado que tenemos 3 clases, este enfoque entrenaría 3 modelos de clasificación binaria:
\begin{itemize}
    \item Premium Lager vs (IPA + Light Lager)
    \item IPA vs (Premium Lager + Light Lager)
    \item Light Lager vs (Premium Lager + IPA)
\end{itemize}
Para hacer una predicción, se utiliza un sistema de votación donde cada modelo de clasificación binaria emite un voto por la clase que predice, y la clase con más votos es la predicción final.

Para la implementación de estos enfoques utilizando sklearn, se puede utilizar la clase \textbf{OneVsOneClassifier} para el enfoque One-vs-One y la clase \textbf{OneVsRestClassifier} para el enfoque One-vs-Rest. Ambos enfoques se pueden implementar utilizando el modelo KNN como clasificador base. La manera en la que esto funciona es inicializando la clase \textbf{OneVsOneClassifier} o \textbf{OneVsRestClassifier} y pasando como argumento el modelo KNN, luego se ajusta el modelo con los datos de entrenamiento y se realizan las predicciones con los datos de prueba.

\begin{lstlisting}
from sklearn.multiclass import OneVsOneClassifier, OneVsRestClassifier

# One-vs-One
ovo_model = OneVsOneClassifier(KNeighborsClassifier(n_neighbors=k))

# One-vs-Rest
ovr_model = OneVsRestClassifier(KNeighborsClassifier(n_neighbors=k))
\end{lstlisting}
...
Otro cambio en la implementación es la modificación a la clase que se tenía para entrenar/probar KNN multiclase, añadiendo como parámetro el tipo de enfoque a utilizar, siendo este \texttt{'ovo'} para One-vs-One y \texttt{'ovr'} para One-vs-Rest.
...
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Estrategia} & \textbf{Train Acc} & \textbf{Train F1} & \textbf{Test Acc} & \textbf{Test F1} \\
\hline
...
Al analizar las matrices de confusión para los 3 enfoques (Figura \ref{fig:p6_cm_ovo_ovr}), se aprecia que todos cometieron el mismo error de clasificación, clasificando incorrectamente 10 muestras de la clase Premium Lager como Light Lager, lo que se refleja en una matriz de confusión con un único error de clasificación para ambas estrategias.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{images/p6_cm_ovo_ovr.png}
    \caption{Matrices de confusión para los tres enfoques (Standard, One-vs-Rest, One-vs-One) con $k=5$. Todos muestran el mismo patrón: 10 muestras de Premium Lager mal clasificadas como Light Lager.}
    \label{fig:p6_cm_ovo_ovr}
\end{figure}

\subsection{Discusión}

\textbf{Equivalencia de estrategias:} Los resultados idénticos entre las tres estrategias (estándar, OvO, OvR) se pueden explicar por el hecho de que el dataset es relativamente sencillo de clasificar. El modelo KNN es capaz de clasificar correctamente las muestras de prueba con cualquier enfoque, ya que las clases están relativamente bien separadas en el espacio de características.

\textbf{Impacto de la división train-test:} La estrategia de aumentar drásticamente el tamaño del conjunto de prueba (10/90 en lugar de 80/20) se hizo con el fin de evitar los problemas de sobreajuste que se presentaron en el Problema 5. Esta modificación permitió obtener resultados más realistas (Accuracy de 0.9259 vs 1.0) que reflejan mejor la capacidad del modelo para generalizar a datos no vistos con solo 5 muestras de entrenamiento por clase.

\textbf{Patrón de errores consistente:} Los 10 errores de clasificación ocurrieron consistentemente entre Premium Lager y Light Lager en todos los enfoques. Esto refuerza la observación del Problema 5 sobre la similitud entre estos dos estilos de lager, mientras que la IPA se distingue fácilmente por sus características más distintivas (mayor IBU y ABV).

\textbf{Robustez del modelo:} El hecho de que el modelo mantenga un F1 Score de 0.9250 con solo 15 muestras de entrenamiento demuestra que las características del dataset (OG, ABV, pH, IBU) son lo suficientemente informativas para permitir buena clasificación incluso con muy pocos ejemplos por clase.
...
\begin{enumerate}
    \item Los tres enfoques (estándar, OvO, OvR) producen resultados idénticos, tanto en métricas como en patrones de error.
    \item La división 10/90 proporcionó una evaluación más realista de la generalización, evitando los resultados perfectos del Problema 5.
    \item Los errores ocurrieron consistentemente entre Premium Lager y Light Lager, confirmando la similitud entre estos estilos.
    \item El modelo KNN demostró robustez significativa, manteniendo desempeño superior al 92\% con solo 5 muestras de entrenamiento por clase.
\end{enumerate}
...
\begin{itemize}
    \item El dominio de $x$ es el conjunto de los números reales $\mathbb{R}$.
    \item El dominio de los coeficientes $a$, $b$ y $c$ es el conjunto de los números reales $\mathbb{R}$, con la restricción de que $a \neq 0$, ya que de lo contrario no sería una ecuación cuadrática al eliminar el término cuadrático por el cual está multiplicando.
    \item Para clasificar las raíces de la ecuación cuadrática, se utilizará el discriminante, que se define como $\Delta = b^2 - 4ac$. La clasificación de las raíces se realiza de la siguiente manera:
    \begin{itemize}
        \item Si $\Delta > 0$, la ecuación tiene dos raíces reales y distintas.
        \item Si $\Delta \approx 0$, la ecuación tiene una raíz real doble (o dos raíces reales iguales).
        \item Si $\Delta < 0$, la ecuación no tiene raíces reales, sino que tiene dos raíces complejas conjugadas.
    \end{itemize}
    \item A los coeficientes $a$, $b$ y $c$ se les asignarán valores aleatorios dentro de un rango específico para generar un conjunto de datos sintéticos. Para este caso se eligió el rango de $-10$ a $10$ para cada uno de los coeficientes (asegurándonos de que $a \neq 0$).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Clase 0:} Raíces complejas conjugadas ($\Delta < 0$)
    \item \textbf{Clase 1:} Raíz real doble ($\Delta \approx 0$)
    \item \textbf{Clase 2:} Raíces reales y distintas ($\Delta > 0$)
\end{itemize}

\textbf{Generación del dataset sintético:}
...
Esta función genera 3000 ecuaciones cuadráticas (1000 por clase), asegurando que $|a| \geq 0.1$ para evitar ecuaciones degeneradas. Para la Clase 1, se añade una pequeña perturbación ($\pm 0.05$) alrededor de $c = b^2/(4a)$ para simular condiciones cercanas al discriminante cero.

\textbf{Preprocesamiento de datos:}

A estos datos solamente se les realizó un preproceso de división en conjuntos de entrenamiento (80\%) y prueba (20\%), y luego se normalizaron utilizando \texttt{StandardScaler} para mejorar el rendimiento del modelo KNN. No fue necesario realizar imputación de valores faltantes ni codificación de variables categóricas, ya que todas las características son numéricas y el dataset se generó sin valores faltantes.

\textbf{Entrenamiento del modelo:}

Finalmente, se entrenó el modelo KNN con los datos de entrenamiento utilizando la clase \textbf{KNNQuadraticClassifier} que encapsula la funcionalidad del modelo:
...
\begin{itemize}
    \item \textbf{Train:} Accuracy = 1.0000, F1 Score = 1.0000
    \item \textbf{Test:} Accuracy = 0.8950, F1 Score = 0.8954
\end{itemize}

La Figura \ref{fig:p7_best_k} muestra la evolución del F1 Score en función del valor de $k$. Se observa que $k=1$ proporciona el mejor rendimiento, y conforme aumenta el valor de $k$, el rendimiento del modelo disminuye gradualmente. Esto sugiere que considerar solo el vecino más cercano es suficiente para este problema, y agregar más vecinos introduce ruido que perjudica la clasificación.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/p7_best_k.png}
    \caption{Evolución del F1 Score en función del hiperparámetro $k$ para el modelo KNN en clasificación de raíces cuadráticas. Se observa que $k=1$ proporciona el mejor rendimiento, con degradación gradual al aumentar $k$.}
    \label{fig:p7_best_k}
\end{figure}

La matriz de confusión (Figura \ref{fig:p7_cm}) revela el patrón de clasificación del modelo. Se aprecia que la mayoría de los aciertos se concentran en la diagonal principal, con cada clase teniendo más de 170 muestras correctamente clasificadas de un total de 200 muestras por clase (600 en total para prueba).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/p7_cm.png}
    \caption{Matriz de confusión para KNN con $k=1$. Las tres clases muestran buen desempeño, aunque la Clase 1 (raíz real doble) presenta mayor cantidad de errores, principalmente confundida con la Clase 2 (raíces reales distintas).}
    \label{fig:p7_cm}
\end{figure}
...
\begin{itemize}
    \item \textbf{Clase 0 (Raíces complejas):} 183 correctas, 17 confundidas con Clase 1, 0 confundidas con Clase 2.
    \item \textbf{Clase 1 (Raíz real doble):} 172 correctas, 19 confundidas con Clase 0, 9 confundidas con Clase 2.
    \item \textbf{Clase 2 (Raíces reales distintas):} 182 correctas, 2 confundidas con Clase 0, 16 confundidas con Clase 1.
\end{itemize}
...
\begin{enumerate}
    \item La elección de $k=1$ resultó ser la mejor opción para este conjunto de datos sintético, indicando que el vecino más cercano proporciona la información más relevante para la clasificación.
    \item El enfoque basado en los coeficientes $(a, b, c)$ como características demostró ser efectivo para predecir el tipo de raíces, validando que el discriminante $\Delta = b^2 - 4ac$ captura implícitamente información relevante a través de estos coeficientes.
    \item La Clase 1 (raíz real doble, $\Delta \approx 0$) presentó mayor dificultad de clasificación, lo cual es matemáticamente consistente al representar una frontera continua entre dos regiones discretas.
    \item La matriz de confusión reveló que los errores están distribuidos de manera relativamente uniforme entre las clases, sin sesgos significativos hacia ninguna clase en particular.
    \item El modelo alcanzó un rendimiento robusto del 89.54\% en un problema que involucra fronteras matemáticas naturalmente difusas, demostrando la viabilidad del enfoque de machine learning para problemas de clasificación matemática.
\end{enumerate}
