...
\begin{itemize}
    \item Make: 39
    \item Model: 715
    \item Vehicle Class: 14 
    \item Transmission: 23
    \item Fuel Type: 4
\end{itemize}
...
Las variables categóricas se transformaron mediante codificación \textit{one-hot} utilizando \texttt{get\_dummies} de la libreria pandas, eliminando una categoría por variable para evitar colinealidad.
...
Los resultados indican que un modelo lineal, utilizando un conjunto reducido de variables cualitativas y cuantitativas relacionadas al tipo de vehículo, es capaz de explicar generalizar de buena manera el consumo que estos podrian tener tanto en ciudad, como en carretera.
...
Se seleccionaron las mismas variables explicativas: tamaño del motor, número de cilindros, clase del vehículo, tipo de transmisión y tipo de combustible. Las variables categóricas fueron codificadas mediante \textit{one-hot encoding}, eliminando una categoría por variable para evitar colinealidad.
...
Los resultados obtenidos mediante ecuaciones normales son practicamente idénticos a los obtenidos en el Problema 1, tanto para consumo en ciudad como para consumo en carretera.
...
La igualdad de las métricas confirma que ambos enfoques resuelven el mismo
...
Se reutilizó el mismo conjunto de datos del Problema 3 y el mismo grado polinomial ($n=12$). La diferencia principal fue que aquí se usó la implementación de \texttt{sklearn} para generar automáticamente las características polinomiales y ajustar el modelo lineal en un \texttt{Pipeline}. Esto evita la implementación manual del descenso por gradiente y delega la solución a un método numéricamente estable.
...
Se reutilizó el conjunto de datos del Problema 3, pero reemplazando el modelo polinomial por un modelo lineal (\texttt{LinearRegression}). La parte nueva de este problema fue el uso de \texttt{learning\_curve} para graficar curvas de aprendizaje con validación cruzada ($cv=5$), midiendo MSE.
...
La implementación utiliza \texttt{PolynomialFeatures} para expandir la variable $x$ y \texttt{SGDRegressor} para entrenar iterativamente con un número fijo de iteraciones (1000), lo que permite observar el efecto del entrenamiento.
...
Se calcularon curvas de aprendizaje con \texttt{learning\_curve} (nueva parte relevante en esta sección) usando validación cruzada ($cv=5$) y MSE.
...
Se reutilizó el pipeline del Problema 7 para construir características polinomiales ($n=20$) y estandarizarlas. El cambio principal fue sustituir el modelo basado en SGD por regresión \texttt{Ridge}, incorporando regularización L2 con $\alpha=1.0$.
...
Se mantuvo la misma configuración del Problema 9 (características polinomiales $n=20$ y estandarización). El cambio principal fue sustituir \texttt{Ridge} (L2) por \texttt{Lasso} (L1), con $\alpha=0.1$, para observar el efecto de una regularización que además tiende a hacer cero algunos coeficientes.
